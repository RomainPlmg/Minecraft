...
class ChunkMesher {
   public:
    explicit ChunkMesher(const BlockRegistry& registry) : m_registry(registry), m_greedy_mesher(registry) { m_builder.reset(); }
   ...
   private:
    ...
    GreedyMesher m_greedy_mesher;
    std::array<BlockType, (Chunk::CHUNK_WIDTH + 2) * Chunk::CHUNK_HEIGHT*(Chunk::CHUNK_WIDTH + 2)> m_grid;
